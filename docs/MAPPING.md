...
| (no Acquia doc — SearchStax installs the config set on the collection)                                                                        | `solrconfig`      | `drush php:script <export-solr-config.php>` via `search_api_solr.configset_controller`, then `GET <core>/schema/name` to confirm the collection runs it |
